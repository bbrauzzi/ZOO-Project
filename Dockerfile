...
# to build:
# docker build --rm -t bbrauzzi/zookernel:latest .
# to run:
# docker run --rm -ti -p 80:80  bbrauzzi/zookernel:latest
...
RUN conda --version
RUN conda create -n ades-dev python=3.8 workflow-executor=1.0.23 cookiecutter -y -c conda-forge -c eoepca
...
########################################
# ZOO_KERNEL
RUN cd /opt && git clone https://github.com/bbrauzzi/ZOO-Project.git -b feature/user-workspace
...
COPY zoo-project/zoo-kernel/scripts/prepareUserSpace.sh /opt/ZOO-Project/zoo-project/zoo-kernel/scripts/prepareUserSpace.sh
RUN chmod +x /opt/ZOO-Project/zoo-project/zoo-kernel/scripts/prepareUserSpace.sh
